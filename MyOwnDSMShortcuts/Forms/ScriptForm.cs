...
namespace BeatificaBytes.Synology.Mods
{
    ...
    public partial class ScriptForm : Form
    {
        ...
        public ScriptForm(ScriptDetails script1, ScriptDetails script2, List<Tuple<string, string>> variables)
        {
            ...
            if (this.scriptTab1 != null)
            {
                if (this.scriptTab1.Code == null)
                {
                    this.scriptTab1.Code = "#!/bin/sh\n";
                }
                tabPageScript1.Text = this.scriptTab1.Title;
            ...
            {
                tabPageScript2.Text = scriptTab2.Title;
            }
        ...
        private void ScriptForm_Load(object sender, EventArgs e)
        {
            ...
            if (scriptTab1 != null)
            {
                scintillaScriptTab1 = new ScintillaNET.Scintilla();
                tabControl.TabPages[tab].Controls.Add(scintillaScriptTab1);
                scintillaScriptTab1.Text = scriptTab1.Code;

                InitScriptEditor(scintillaScriptTab1, Lexer.Batch);
                tab++;
            }
            else
            ...
            if (scriptTab2 != null)
            {
                scintillaScriptTab2 = new ScintillaNET.Scintilla();
                tabControl.TabPages[tab].Controls.Add(scintillaScriptTab2);
                scintillaScriptTab2.Text = scriptTab2.Code;

                InitScriptEditor(scintillaScriptTab2, Lexer.PhpScript);
            }
            else
                tabControl.TabPages.RemoveAt(tab);
        ...
        private void GetSynoEnvVariables(List<Tuple<string, string>> variables)
        {
            variables.Add(new Tuple<string, string>("SYNOPKG_PKGNAME", "Package identify which is defined in INFO"));
            ...
            variables.Add(new Tuple<string, string>("SYNOPKG_TEMP_LOGFILE", "A temporary file path for a script to log information or error messages"));
        ...
        private void listBoxVariables_MouseDoubleClick(object sender, MouseEventArgs e)
        {

            if (listBoxVariables.SelectedIndex != -1)
            {
                var rect = listBoxVariables.GetItemRectangle(listBoxVariables.SelectedIndex);
                if (rect.Contains(e.Location))
                {
                    var text = listBoxVariables.SelectedItem as string;
                    text = text.Split(':')[0];
